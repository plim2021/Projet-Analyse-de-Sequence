...
void score(char* seq1[], char * seq2[]){
    ...
    extract_sequence(const char* path_input, char* sequence1);
    printf("Entrez le nom du fichier de la deuxième séquence à comparer\n"); 

    get_path_from_user(char *path_input);
    extract_sequence(const char* path_input, char* sequence2);

    int taille_seq1 = strlen(sequence1); // taille de la séquence 1
    int taille_seq2 = strlen(sequence2); // taille de la séquence 2
    ...
    if (taille_seq1 == taille_seq2){
        ...
        printf("seq1 %s \n", &sequence1[taille_seq1]);
        printf("seq2 %s \n", &sequence2[taille_seq2]);
        printf("-id- %s \n", id[identite]);
